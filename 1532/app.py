...
def record_event(ev_type: str, query: str = "", target: str = ""):
    pid = _get_prolific_id()
    group, _ = choose_group_and_dir(pid)
    header = ["timestamp", "prolific_id", "type", "query", "target", "分组"]
    _ensure_csv(EVENTS_LOG, header)
    row = [_now(), pid, ev_type, _clean(query, 2000), _clean(target, 2000), str(group)]
...
def guess_title_and_text_and_url(html_path):
    try:
        ...
        return (os.path.basename(html_path), "", None)
    ...
    # text (truncate later, keep some content)
    ...
    # fallback to internal served page
    if not url:
        url = url_for("serve_page", dir=os.path.basename(os.path.dirname(html_path)), name=os.path.basename(html_path), _external=False)
...
def load_pages_from_dir(dir_path, limit=60):
    ...
    for fp in files[:limit]:
        title, text, url = guess_title_and_text_and_url(fp)
        pages.append({
            "title": title,
            "text": text,
            "href": url,
...
def generate_overview(query, pages, max_sources=8):
    ...
    for i, p in enumerate(ranked, start=1):
        href = p.get("href") or ""
        # route clicks via /out for logging
        out_url = url_for("out_click", u=href, title=p.get("title",""), q=query, _external=False)
...
def results():
    ...
    if q:
        ...
        try:
            ...
            overview_text = None
        record_event("overview", q, f"{len(pages)} candidates from {os.path.basename(dir_path)}")
...
@app.route("/out")
def out_click():
    # Log click and redirect to target (external or internal /page)
    ...
    if not url:
        return redirect(url_for("results"))
    record_event("click", q, url or title)
...
def admin_events_download():
    ...
    if not os.path.exists(EVENTS_LOG):
        _ensure_csv(EVENTS_LOG, ["timestamp","prolific_id","type","query","target","分组"])
